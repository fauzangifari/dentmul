...
import { updateOwnDosenProfile } from "@/features/dosen/actions";

type DosenProfileFormValues = {
  name: string;
  noTelepon: string;
};

type DosenProfileUser = {
  name: string;
  email: string;
  noTelepon: string;
};

export function DosenProfileForm({ user }: { user: DosenProfileUser }) {
  ...
  const onSubmit = (values: DosenProfileFormValues) => {
    startTransition(async () => {
      const result = await updateOwnDosenProfile(values);
      if (result?.error) {
